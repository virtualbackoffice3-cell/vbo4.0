...
function render() {
  ...
  if (!rows.length) {
    els.taskBody.innerHTML = `<tr><td colspan="13" class="cell-muted">No rows found</td></tr>`;
...
<td>${Array.from(new Set([
  ...
  valueOf(row, "calling_phone")
    ...
    tr.children[12].appendChild(createdInput);
...
function renderUnallocated() {
  ...
  rows.forEach((row, index) => {
    ...
    tr.innerHTML = `
      ...
      <td></td>
    `;
    tr.children[5].textContent = slaStatus(row, row.log_timestamp);
    tr.children[6].appendChild(startInput);
    tr.children[7].appendChild(teamPicker);
    tr.children[8].appendChild(saveButton);
    tr.children[9].appendChild(closeInput);
...
function downloadCsv() {
  ...
  } else if (state.view === "forceclosed") {
    filename = "forceclosed-tasks.csv";
