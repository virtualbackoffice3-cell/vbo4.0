(() => {
    ...
    const DEMO_TOAST_MESSAGE = "Its demo account!";
    const coreOptions = [
        "1,Blue",
        "2,Orange",
        "3,Green",
    ...
    function createShell() {
        elements.mount.innerHTML = `
            <div class="jc-note-root">
                ...
                <div class="jc-demo-toast" id="jcDemoToast" aria-live="polite" aria-atomic="true"></div>
            </div>
        `;
        ...
        elements.demoToast = state.root.querySelector("#jcDemoToast");
    ...
    function showDemoToast() {
        if (!elements.demoToast) return Promise.resolve();
        const toast = elements.demoToast;
        toast.textContent = DEMO_TOAST_MESSAGE;
    ...
    function runPartialPonWorkflow(config) {
        return new Promise(async (resolve, reject) => {
            ...
            const demoMode = !!(config && config.demoMode);
            if (!windowName || !oltpon || (!cuid && !demoMode)) {
                ...
                try {
                    ...
                    if (demoMode) {
                        ...
                        resolve({
                            ...
                            CUID: Number(cuid),
                            latest_timestamp: currentTimestamp,
                            partialpon: Array.isArray(fetchResult && fetchResult.mac_addresses) ? fetchResult.mac_addresses.join(",") : "",
                            users: [],
                            mac_addresses: Array.isArray(fetchResult && fetchResult.mac_addresses) ? fetchResult.mac_addresses : [],
                            count: Number(fetchResult && fetchResult.count || 0)
                        });
                        return;
                    }
