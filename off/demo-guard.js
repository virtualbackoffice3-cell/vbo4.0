(() => {
    ...
    const WINDOW_LABELS = {
        ...
        "TEAMAMANWIZ": "Demo Team 01",
        "TEAMMEDANTA": "Demo Team 02",
        "TEAMSEVAI": "Demo Team 03"
    };

    function showDemoToast(message = DEMO_MESSAGE, success = false) {
        ...
        toast.style.background = success ? "#52c41a" : "rgba(0,0,0,0.85)";
    ...
    function maskByKey(key, value) {
        ...
        if (normalizedKey === "_window" || normalizedKey === "window") return value;
        if (normalizedKey === "pon") return maskPon(value);
        ...
        if (normalizedKey.includes("phone") || normalizedKey.includes("registered") || normalizedKey.includes("calling") || normalizedKey.includes("called") || normalizedKey === "number") {
        ...
        if (normalizedKey.includes("mac") || normalizedKey.includes("serial")) {
            return maskMac(value);
        }
        if (normalizedKey.includes("ticket") || normalizedKey.includes("complaint")) {
    ...
    window.fetch = async function demoFetch(input, options = {}) {
        if (isWriteRequest(input, options)) {
            showDemoToast("Demo record saved for walkthrough.", true);
            return new Response(JSON.stringify({ ok: true, demo: true, message: DEMO_MESSAGE, id: "DEMO-RECORD" }), {
                status: 200,
    ...
    window.addEventListener("DOMContentLoaded", () => {
        ...
        document.addEventListener("click", (event) => {
            const csvButton = event.target && event.target.closest ? event.target.closest("#btnCsv") : null;
            if (!csvButton) return;
